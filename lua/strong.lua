function Block(el)
    if el.t == "Para" or el.t == "Plain" then
        for k, _ in ipairs(el.content) do
            ...
            if el.content[k].t == "Str" and el.content[k].text == "Subedi," and
                ...
                el.content[k] = pandoc.Strong {pandoc.Str("Subedi, M.R.")}
                el.content[k + 1] = pandoc.Str(rest)
                table.remove(el.content, k + 2)
				--table.remove(el.content, k + 3)
				--table.remove(el.content, k + 4)-- safe? another way would be to set element k+2 to Str("")
...
--              local _, e = el.content[k + 2].text:find("^M.")
  --              local rest = el.content[k + 2].text:sub(e + 1) -- empty if e+1>length
    --            el.content[k] = pandoc.Strong {pandoc.Str("Subedi, M.")}
      --          el.content[k + 1] = pandoc.Str(rest)
